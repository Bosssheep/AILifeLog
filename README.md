...
- 将顶部圆点筛选迁移到列表与标签页侧边栏，并保持滚动可见（sticky）
- 保证 `list` 卡片区域与 `tag` 时间线区域宽度对齐，减少误判
- 新增“查看所有卡片”圆点，点击回到列表并展示全部

### 📅 汇总页 SummaryView：AI 心情评分与情绪曲线（v4.1）

> **更新日期：** 2026-03-25

- 基于 AI 生成的 `reply` block 情绪分值，计算当前范围内的情绪趋势
- 支持按 `日` / `周` / `年度` 切换并渲染心情曲线折线图
- 悬停可查看 tooltip（日期/区间与分值），无 score 时展示空状态文案
...
后端端口：Server listening on http://localhost:4000
